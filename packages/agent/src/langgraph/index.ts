...
import { buildIntention, type IntentionOptions, extractIntentWithFallback, generatePlanningWithOllama } from '@citypass/utils';
...
export interface AgentState {
  ...
  reasons?: string[];
  aiSummary?: string; // Generated by larger Ollama model (Qwen2.5:72b/LLaMA-3:70b)
  traceId: string;
  errors?: string[];
}
...
/**
 * Node: Understand
 * Parse free text and build intention using LLM → Pattern → Defaults fallback
 */
async function understandNode(state: AgentState): Promise<Partial<AgentState>> {
  let extractedTokens: Partial<IntentionTokens> = {};
  ...
  if (state.freeText) {
    ...
    console.log(`✨ Intent extracted via ${extractionMethod}:`, extractedTokens);
  }
...
/**
 * Node: Answer
 * Generate personalized summary using larger Ollama model (Qwen2.5:72b/LLaMA-3:70b)
 */
async function answerNode(state: AgentState): Promise<Partial<AgentState>> {
  // Only generate AI summary if enabled and we have slates
  const useAISummary = process.env.USE_OLLAMA_PLANNING !== 'false';

  if (!useAISummary || !state.slates || !state.intention) {
    return {};
  }

  try {
    // Build context about the slates
    const bestEvents = state.slates.best?.slice(0, 3) || [];
    const wildcardEvents = state.slates.wildcard?.slice(0, 2) || [];
    ...
    const eventsContext = [
      ...
      ...(wildcardEvents.length > 0 ? [
        '\nWildcard options:',
        ...wildcardEvents.map((e, i) =>
          `${i + 1}. ${e.title} at ${e.venueName}`
        )
      ] : [])
    ].join('\n');

    const systemPrompt = `You are a personalized event concierge. Generate a friendly, concise summary (2-3 sentences) explaining why these recommendations fit the user's request. Be enthusiastic but natural.`;

    const userPrompt = `User asked: "${state.freeText || 'events for tonight'}"
Their preferences: ${state.intention.tokens?.mood || 'balanced'} mood, ${state.intention.tokens?.budget || 'casual'} budget

Top recommendations:
${eventsContext}

Generate a 2-3 sentence summary explaining why these events are perfect for them.`;

    const summary = await generatePlanningWithOllama(systemPrompt, userPrompt, {
      temperature: 0.7,
      maxTokens: 150,
    });

    if (summary) {
      console.log('✨ Generated AI summary with large model');
