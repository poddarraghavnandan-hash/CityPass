      ...
      async start(controller) {
        const send = (event: string, data: any) => {
          try {
            ...
            console.warn('[ChatAPI] Failed to send event:', e);
          }
        };
        ...
        try {
          ...
          console.log('[ChatAPI] Starting chat turn for:', { userId, anonId, freeText });
          ...
          // Only if we have fewer than 3 results or user explicitly asked for something new
          const shouldDiscover = normalizedSlates.best.length < 3 || plannerDecision.intention.notes?.includes('discover');
