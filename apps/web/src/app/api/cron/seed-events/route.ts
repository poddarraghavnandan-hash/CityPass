...
/**
 * Vercel Cron Job: Ensure minimum seed inventory
 * Runs every 10 minutes to maintain at least 12 upcoming events per city
 */
...
export async function GET(request: Request) {
  ...
  try {
    const result = await ensureSeedInventory();
    return NextResponse.json({
      success: true,
      ...result,
      message: `Ensured seed inventory: ${result.created} events created`,
    });
  } catch (error: any) {
